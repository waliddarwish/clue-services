...
module.exports = function Graph(serialized){
  ...
  var edgeWeights = {};
    // The information about edge.
  // Keys are string encodings of edges.
  // Values are extra information about how the edges are connected (string).
  var edgeInfo = {};
  ...
  // Sets the info of the given edge
  function setEdgeInfo(u,v, info){
    edgeInfo[encodeEdge(u, v)] = info;
    return graph;
  }
  ...
  // Gets the info of the given edge.
  // Returns "" if no info was previously set.
  function getEdgeInfo(u, v){
    var info = edgeInfo[encodeEdge(u, v)];
    return info === undefined ? "" : info;
  }

  // Adds an edge from node u to node v.
  // Implicitly adds the nodes if they were not already added.
  function addEdge(u, v, weight, info){
    addNode(u);
    addNode(v);
    adjacent(u).push(v);
    ...
    serialized.nodes.forEach(function (node){
      ...
      adjacent(source).forEach(function (target){
        serialized.links.push({
          ...
          info:   getEdgeInfo(source, target)
        });
      });
    });
  ...
  function deserialize(serialized){
    ...
    serialized.links.forEach(function (link){ addEdge(link.source, link.target, link.weight, link.info); });
    return graph;
  }
