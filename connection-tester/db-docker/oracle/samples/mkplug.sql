...
SPOOL &log_path.mkplug_v3._@.log 
...
CREATE USER ix IDENTIFIED BY Oradoc_db1 ;
CREATE USER sh IDENTIFIED BY Oradoc_db1 ;
CREATE USER pm IDENTIFIED BY Oradoc_db1 ;
...
host impdp "'sys/Oradoc_db1@ORCLCDB AS SYSDBA'" directory=SS_IMPEXP_DIR logfile=SS_LOGPATH_DIR:tts_example_imp.log dumpfile=&imp_file transport_datafiles='&datafile' EXCLUDE=trigger,functional_index,index_statistics silent=banner
...
@/u01/samples/order_entry/oe_main.sql &&password_oe EXAMPLE TEMP  &&password_hr &&password_sys &&oe_data_path &&log_path v3
...
SPOOL &log_path.mkplug_p2_v3..log 
...
@/u01/samples/bus_intelligence/bi_views.sql &&password_bi

--
-- set up extra contraints in the PM schema
connect pm/Oradoc_db1@SAMPLES
...
SET CONTAINER
=SAMPLES;
@/u01/samples/product_media/pm_analz
CONNECT hr/Oradoc_db1@SAMPLES
ALTER SESSION
SET CONTAINER
=SAMPLES;
...
SET CONTAINER
=SAMPLES;
@/u01/samples/sales_history/sh_analz
